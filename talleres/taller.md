...
R: Es concurrencia, ya que hay un solo nucleo alternando entre las tareas (entrelazadas), no ejecutandoe al mismo tiempo exacto.
...
R:
MEMORIA: Los procesos tienen espacios de memoria independientes; los hilos comparten la misma memoria del proceso.

COSTO: Los procesos son más pesados de crear y consumir más recursos; los hilos son ligeros.

FALLO: Si un proceso falla o se cuelga, no afecta a los demás; si un hilo no controlado rompe el proceso, caen todos los hilos.


3. ¿En qué estado del ciclo de vida está un hilo que ejecuta `time.sleep(2)`? ¿Qué devuelve `is_alive()`?


R: Devuelve el true, por que sigue vivo aunque no este ejecutando .
...
R: El gil protege el acceso a obgetos internos de python. No evita condiciones de carrera en los datos de tu programa; hay que usar candados (lock) para eso.
...
R-: Se sobreescribe el  run(). nunca se puede el __init__ sin llamar el super().__init__(...), ni sobreescribir start(),por que start() es quien arranca el hilo y llama a run() internamente


7. ¿Qué pasa con un hilo daemon cuando termina el hilo principal? ¿Qué **no** se ejecuta?


R:Cuando terminan el hilo principal , los hilos daemon se cortan en seco sin terminar su trabajo .No ejecutan bloques finally ni limpian recursos 


8. Completar la regla del curso: *hilos para , procesos para*.


R: hilos para : esperar
